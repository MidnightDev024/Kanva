  ...
  return (
    <div className={`bg-[#8185B2]/10 h-full p-5 rounded-r-xl overflow-y-scroll text-white ${selectedUser ? "max-md:hidden" : ''}`}>
      <div className='pb-5'>
        ...
        <div className='bg-[#282142] rounded-full flex item-center gap-2 py-3 px-4 mt-0'>
        ...
        <div className='flex flex-col'>
          {userDummyData.map((user, index) => {
            <div onClick={()=> {setSelectedUser(user)}} key={index} className={`relative flex item-center gap-2 p-2 pl-4 rounded cursor-pointer max-sm:text-sm ${selectedUser?._id === user._id && '[#282142]/50'}`}>
              ...
              {
                index > 2
                && <p className='absolute top-4 right-4 text-xs h-5 w-5 flex justify-center item-center rounded-full bg-voilet-500/50'>{index}</p>
              }
            </div>
          })}
        </div>
