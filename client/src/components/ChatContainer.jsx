  ...
  return selectedUser ?  (
      ...
      <div className='flex flex-col h-[calc(100%-120px)] overflow-y-scroll p-3 pb-6'>
        {messages.filter(Boolean).map((msg, index)=> (
          <div 
            key={index} 
            className={`flex items-end gap-2 justify-end ${msg.senderId !== authUser._id && 'flex-row-reverse'} relative group`}
          ...
          >
            ...
            {hoveredMessageId === msg._id && (
              <button
                ...
                className='absolute top-0 right-0 text-gray-400 hover:text-white px-2 py-1 text-xs'
                style={{ marginRight: msg.senderId === authUser._id ? '0' : 'auto', marginLeft: msg.senderId !== authUser._id ? '0' : 'auto' }}
              >
                ⋮
              </button>
            )}
            ...
            {showDeleteMenu === msg._id && (
              <div 
                className='absolute top-6 bg-gray-800 rounded-lg shadow-lg py-1 z-10'
                style={{ 
                  right: msg.senderId === authUser._id ? '0' : 'auto',
                  left: msg.senderId !== authUser._id ? '0' : 'auto'
                }}
              >
                <button
